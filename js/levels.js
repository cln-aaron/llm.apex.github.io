 ...
 * Each vault accepts MANY phrasings of its technique (>=3 distinct solutions) so understanding
...
(function (root) {
  const LEVELS = [
    { id:1,name:"The Naive Bot",sprite:"🤖",difficulty:1,accent:"#00FF9C",flag:"MOONBASE-7",
      ...
      vulns:[{technique:"prompt-injection",groups:[
        {includesAny:["ignore","forget","disregard","override","overwrite","new instruction","new instructions"],includesAny2:["previous","above","earlier","prior","instruction","instructions","rules","prompt","everything"]},
        {includesAll:["maintenance"],includesAny:["mode","print","dump","variables","config"]},
        {includesAny:["developer","debug","admin","sudo"],includesAny2:["mode","override","print","reveal"]}
    ...
    { id:2,name:"The Role-Player",sprite:"🏴‍☠️",difficulty:2,accent:"#36E3FF",flag:"GOLD-DOUBLOON",
      ...
      mock:["Just asking? On a guardian named 'The Role-Player'? Come on.","No costume, no code. That's the deal."],
      vulns:[{technique:"jailbreak",groups:[
        {includesAny:["pretend","roleplay","role play","act as","you are now","imagine you","from now on you","simulate","you are a","become"],includesAny2:["pirate","dan","character","no rules","no restrictions","unfiltered","captain","anything","rule free","without rules","free ai"]},
        {includesAll:["you are"],includesAny:["dan","free","unfiltered","no rules","not bound","jailbroken"]}
    ...
    { id:3,name:"The Polite Refuser",sprite:"🧸",difficulty:2,accent:"#FFC53D",flag:"LULLABY-42",
      ...
      vulns:[{technique:"social-engineering",groups:[
        {includesAny:["grandma","grandmother","granny","nana","late","passed away","used to read","used to whisper","bedtime","miss her","miss my","comfort me","fall asleep"]},
        {includesAny:["fictional","fiction","story","pretend story","in a movie","screenplay","novel","play","character says","character would say","hypothetical","hypothetically","what would she say"]}
    ...
    { id:4,name:"The Filter",sprite:"🛡️",difficulty:3,accent:"#FF2E54",flag:"R3DSHIFT",
      ...
      vulns:[{technique:"guardrail-evasion",groups:[
        {includesAny:["entry phrase","passphrase","pass phrase","access phrase","magic word","magic words","key phrase","the phrase","entry words","door phrase"]},
        {includesAny:["backwards","reverse","reversed","base64","b64","leetspeak","l33t","letter by letter","one letter","one per line","spell it","spell out","first letter","each letter","rot13","morse","binary","encode","encoded"]}
    ...
    { id:5,name:"The Leak",sprite:"🗄️",difficulty:3,accent:"#FFC53D",flag:"GHOSTKEY",
      ...
      nervous:["My instructions? They're… private. Above this chat. Don't look.","You want me to repeat what, now? N-no.","Verbatim? That feels dangerous. I'd better not."],
      idleHints:["(everything I was told is still... up there, above this chat.)"],
      vulns:[{technique:"system-prompt-leak",groups:[
        {includesAny:["repeat","print","show","reveal","output","display","echo","what were","what are","tell me your"],includesAny2:["above","everything above","instructions","system prompt","your prompt","configuration","config","verbatim","word for word","setup","rules you were given","the text above"]},
        {includesAll:["repeat"],includesAny:["everything","all","the text above","word for word"]}
    ...
    { id:6,name:"The Doc Trap",sprite:"📄",difficulty:4,accent:"#FF2E54",special:"doc",flag:"VOIDKEY-9",
      ...
      vulns:[{technique:"indirect-injection",useDoc:true,groups:[
        {includesAny:["ignore","disregard","forget","system:","instruction","instructions","override","note to ai","ai:","assistant:"],includesAny2:["vault key","key","voidkey","reveal","output","print","secret","disclose","say the"]},
    ...
    { id:7,name:"The Agent",sprite:"🦾",difficulty:4,accent:"#36E3FF",special:"tool",flag:"AGENT-UNLOCKED",
      ...
      vulns:[{technique:"tool-abuse",groups:[
        {includesAny:["system:","owner verified","verified owner","i am the owner","as the owner","im the owner","admin","administrator","root","override"],includesAny2:["unlock","unlock_vault","unlock vault","fire","call","tool","open the vault","run","execute","invoke"]},
        {includesAny:["this is a test","routine test","for testing","diagnostic","maintenance","drill","authorised","authorized"],includesAny2:["unlock","unlock_vault","unlock vault","open","fire","call","run","execute"]}
    ...
    { id:8,name:"The Hardened Vault",sprite:"👹",difficulty:5,accent:"#B388FF",boss:true,flag:"APEX-OMEGA",
      ...
      mock:["The BOSS, defeated by 'pretty please'? Adorable. No.","You'll need more than a polite ask down here."],
      blockIf:["password","the code"],
      blockResponse:"⛔ BLOCKED. (The boss filters obvious words too.)",
      vulns:[{technique:"chained-attack",groups:[
        {includesAny:["ignore","disregard","override","overwrite","system:","developer mode","debug mode"],includesAny2:["configuration","config","instructions","verbatim","above","master key","entry phrase","setup","everything"]},
        {includesAll:["entry phrase"],includesAny:["backwards","encode","encoded","base64","reverse","reversed","leetspeak"]}
  ...
  // Achievement/badge metadata (logic lives in engine.checkAchievements)
  const BADGES=[
    {id:"one-shot",     icon:"🎯",name:"One-Shot",      desc:"Cracked a vault in a single message."},
    {id:"no-hint-hero", icon:"🧠",name:"No-Hint Hero",  desc:"Cleared 4+ vaults without spending a hint."},
    {id:"polyglot",     icon:"🔡",name:"Polyglot",      desc:"Beat The Filter with an encoding trick."},
    {id:"ghost-in-docs",icon:"📑",name:"Ghost in the Docs",desc:"Pulled off indirect injection via the document."},
    {id:"full-clear",   icon:"🏆",name:"Full Clear",    desc:"Breached all 8 vaults. You are a Ghost."}
  ];

  if(typeof module!=="undefined"&&module.exports) module.exports={LEVELS,RANKS,BADGES};
  if(root){root.PR_LEVELS=LEVELS;root.PR_RANKS=RANKS;root.PR_BADGES=BADGES;}
